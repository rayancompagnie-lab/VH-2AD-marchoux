...
// Dans votre fichier src/utils/bibleApi.js ou directement dans Bible.jsx

const BASE_URL = 'https://api.midvash.com/v1';

export async function fetchVersions() {
  try {
    // On demande uniquement les versions en français
    const res = await fetch(`${BASE_URL}/versions?language=fr`);
    if (!res.ok) throw new Error(`Erreur API (${res.status})`);
    
    const json = await res.json();
    // La structure de réponse contient un tableau dans 'data'
    return json.data || [];
  } catch (error) {
    console.error("Impossible de récupérer les versions bibliques:", error);
    return [];
  }
}